...
const emptyForm = (): Omit<Product, 'id' | 'margin' | 'price' | 'createdAt'> => ({
  name: '', description: '', category: 'Cocteles', image: '/assets/trago.png',
  ...
  return (
    <div className={cn(
      ...
      <div className="relative h-40 bg-muted flex items-center justify-center overflow-hidden">
        <Image
          src={product.image || '/assets/trago.png'}
          alt={product.name}
          fill
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-popover border border-border rounded-2xl p-6 animate-scale-in text-center">
        ...
        <p className="text-sm text-muted-foreground mb-1">
          ¿Eliminar <span className="font-semibold text-foreground">"{product.name}"</span>?
        </p>
...
export default function ProductosPage() {
  ...
  const role = (user?.role ?? 'empleado') as 'superadmin' | 'administrador' | 'empleado';

  const canCreate = can(role, 'productos', 'create');
  const canEdit   = can(role, 'productos', 'edit');
  const canDelete = can(role, 'productos', 'delete');
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="relative w-9 h-9 rounded-lg overflow-hidden bg-muted flex-shrink-0">
                            <Image src={p.image || '/assets/trago.png'} alt={p.name} fill className="object-cover" sizes="36px" />
                          </div>
                          <div>
