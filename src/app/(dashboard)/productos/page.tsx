...
const CATEGORIES: ProductoCategoria[] = ['Cocteles', 'Cervezas', 'Destilados', 'Vinos', 'Snacks', 'Otro'];
const PAGE_SIZE = 20;
...
const emptyForm = (): ProductoForm => ({
  codigo: '', nombre: '', descripcion: '', categoria: 'Cocteles',
  precioVenta: 0, precioCosto: 0, disponiblePos: true, activo: true,
});
    ...
    <div className={cn(
      ...
      <div className="p-3 flex flex-col flex-1 gap-2">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground/70 font-mono mt-0.5">{product.codigo}</p>
          {product.descripcion && (
            <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{product.descripcion}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-3 gap-1 text-center">
          <div className="rounded-lg bg-muted/50 p-1.5">
            ...
            <p className="text-xs font-bold text-amber-500">{fmt(product.precioVenta)}</p>
          </div>
          <div className="rounded-lg bg-muted/50 p-1.5">
            <p className="text-[9px] text-muted-foreground uppercase tracking-wider">Costo</p>
            <p className="text-xs font-semibold text-foreground">{fmt(product.precioCosto)}</p>
          </div>
          ...
          <div className="flex gap-1.5 pt-1 border-t border-border">
            {canEdit && (
              <>
                ...
                >
                  {product.activo ? <ToggleRight size={15} className="text-emerald-500" /> : <ToggleLeft size={15} />}
                </button>
                <button
                  onClick={onTogglePOS}
                  className={cn(
...
function ProductModal({
  open, product, saving, error, onClose, onSave,
}: {
  open: boolean;
  product?: Producto;
  saving: boolean;
  ...
  const [form, setForm] = useState<ProductoForm>(() => product
    ? {
        codigo: product.codigo, nombre: product.nombre, descripcion: product.descripcion ?? '',
        categoria: (product.categoria as ProductoCategoria) ?? 'Cocteles',
        precioVenta: product.precioVenta, precioCosto: product.precioCosto,
        disponiblePos: product.disponiblePos, activo: product.activo,
      }
    : emptyForm()
  );
  ...
  const margin = form.precioVenta > 0
    ? Math.round(((form.precioVenta - form.precioCosto) / form.precioVenta) * 100)
    : 0;

  const set = <K extends keyof ProductoForm>(k: K, v: ProductoForm[K]) =>
    setForm((f) => ({ ...f, [k]: v }));

  const valid = form.codigo.trim() !== '' && form.nombre.trim() !== '' && form.precioVenta > 0;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-popover border border-border rounded-2xl shadow-2xl animate-scale-in overflow-y-auto max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border sticky top-0 bg-popover z-10">
          ...
          <button onClick={onClose} disabled={saving} className="text-muted-foreground hover:text-foreground transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => set('categoria', e.target.value as ProductoCategoria)}
...
/* ─── DELETE CONFIRM ─── */
function DeleteModal({ product, saving, error, onClose, onConfirm }: {
  product: Producto; saving: boolean; error: string | null; onClose: () => void; onConfirm: () => void;
}) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => !saving && onClose()} />
      <div className="relative w-full max-w-sm bg-popover border border-border rounded-2xl p-6 animate-scale-in text-center">
        ...
        <p className="text-sm text-muted-foreground mb-1">
          ¿Eliminar <span className="font-semibold text-foreground">&ldquo;{product.nombre}&rdquo;</span>?
        </p>
        <p className="text-xs text-muted-foreground mb-5">Se marcará como inactivo (baja lógica).</p>
        {error && <p role="alert" className="text-xs text-destructive mb-4">{error}</p>}
        <div className="flex gap-3">
          <button onClick={onClose} disabled={saving} className="flex-1 h-10 rounded-xl bg-muted/60 border border-border text-foreground text-sm hover:bg-muted transition-colors disabled:opacity-50">
            Cancelar
          </button>
          <button onClick={onConfirm} disabled={saving} className="flex-1 h-10 rounded-xl bg-red-500 hover:bg-red-400 text-white font-bold text-sm transition-all disabled:opacity-50">
            {saving ? '…' : 'Eliminar'}
          </button>
        </div>
      </div>
...
export default function ProductosPage() {
  ...
  const [catFilter, setCatFilter] = useState<'Todos' | ProductoCategoria>('Todos');
  ...
  useEffect(() => {
    ...
    if (catFilter !== 'Todos') query.categoria = catFilter;
    if (statusFilter !== 'all') query.activo = statusFilter === 'active' ? 'true' : 'false';
  ...
  const quickUpdate = async (p: Producto, patch: UpdateProductoPayload) => {
    setProductos((list) => list.map((it) => (it.id === p.id ? { ...it, ...patch } : it)));
    try {
  ...
  const openCreate = () => { setModalError(null); setCreateOpen(true); };
  const openEdit = (p: Producto) => { setModalError(null); setEditProduct(p); };
  const openDelete = (p: Producto) => { setModalError(null); setDeleteTarget(p); };
  ...
  const categoryTabs: Array<'Todos' | ProductoCategoria> = ['Todos', ...CATEGORIES];
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--background)' }}>
      <div className="p-3 sm:p-4 lg:p-6 space-y-4 lg:space-y-5">
        ...
        {/* KPIs */}
        <Bones name="productos-kpis" loading={loading} placeholder={<BoneKpis count={4} />}>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 stagger-children">
            {[
              { label: 'TOTAL', value: String(total), color: 'text-foreground', icon: <Package size={16} /> },
              { label: 'ACTIVOS (PÁGINA)', value: String(productos.filter((p) => p.activo).length), color: 'text-emerald-600 dark:text-emerald-400', icon: <CheckCircle2 size={16} /> },
              { label: 'EN POS (PÁGINA)', value: String(productos.filter((p) => p.disponiblePos).length), color: 'text-amber-500', icon: <ShoppingCart size={16} /> },
              { label: 'MARGEN PROM. (PÁGINA)', value: (productos.length ? Math.round(productos.reduce((s, p) => s + p.margin, 0) / productos.length) : 0) + '%', color: 'text-blue-500', icon: <TrendingUp size={16} /> },
            ].map((k) => (
        ...
        <div className="flex flex-col sm:flex-row gap-3 animate-fade-in-up">
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-0.5 flex-wrap">
            {categoryTabs.map((c) => (
              <button
                key={c}
                onClick={() => { setCatFilter(c); setPagina(1); }}
                className={cn(
          ...
          <select
            value={statusFilter}
            onChange={(e) => { setStatusFilter(e.target.value as 'all' | 'active' | 'inactive'); setPagina(1); }}
        ...
        {/* CONTENT */}
        <Bones name="productos-contenido" loading={loading} placeholder={<SkeletonProductGrid count={PAGE_SIZE} />}>
          {productos.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 stagger-children">
              {productos.map((p) => (
                <ProductCard
                  ...
                  onTogglePOS={() => quickUpdate(p, { disponiblePos: !p.disponiblePos })}
                />
              ))}
            </div>
          ) : (
              ...
              <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {productos.map((p) => (
                      <tr key={p.id} className={cn('hover:bg-muted/30 transition-colors', !p.activo && 'opacity-50')}>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className="relative w-9 h-9 rounded-lg overflow-hidden bg-muted flex-shrink-0">
                              <Image src="/assets/trago.webp" alt={p.nombre} fill className="object-cover" sizes="36px" />
                            </div>
                            <div>
                              <p className="font-medium text-foreground text-sm">{p.nombre}</p>
                              <p className="text-[10px] text-muted-foreground/70 font-mono">{p.codigo}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="px-2 py-0.5 rounded-full bg-muted text-muted-foreground text-xs">{p.categoria}</span>
                        </td>
                        <td className="px-4 py-3 font-semibold text-amber-500 font-mono">{fmt(p.precioVenta)}</td>
                        <td className="px-4 py-3 text-foreground font-mono">{fmt(p.precioCosto)}</td>
                        <td className="px-4 py-3">
                          <span className={cn(
                            'font-bold',
                        ...
                        <td className="px-4 py-3">
                          <span className={cn(
                            ...
                            p.disponiblePos ? 'bg-amber-500/10 text-amber-500' : 'bg-muted text-muted-foreground'
                          )}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            {canEdit && (
                              <>
                                <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg bg-muted/60 hover:bg-muted text-foreground transition-colors"><Pencil size={13} /></button>
                                <button onClick={() => quickUpdate(p, { activo: !p.activo })} className="p-1.5 rounded-lg bg-muted/60 hover:bg-muted text-muted-foreground transition-colors">
                                  {p.activo ? <ToggleRight size={15} className="text-emerald-500" /> : <ToggleLeft size={15} />}
                                </button>
                              </>
                            )}
                            {canDelete && (
                              <button onClick={() => openDelete(p)} className="p-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-500 transition-colors"><Trash2 size={13} /></button>
                            )}
                          </div>
                        </td>
                      </tr>
